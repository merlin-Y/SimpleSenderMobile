...
class DSManager(context : Context) {
    ...
    fun<U> readData(key: String, default: U): Flow<U>{
    ...
    private fun getIntFlow(key: Preferences.Key<Int>,default : Int = 0): Flow<Int> =
        dataStore.data
            .catch {
                if(it is IOException){
                    it.printStackTrace()
    ...
    private fun getLongFlow(key: Preferences.Key<Long>,default : Long = 0L): Flow<Long> =
        dataStore.data
            .catch {
                if(it is IOException){
                    it.printStackTrace()
